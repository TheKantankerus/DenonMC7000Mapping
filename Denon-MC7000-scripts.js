...
MC7000.encodeColor = (hexColor) => {
  // Ensure number (handles inputs like 0xRRGGBB or 0x000000)
  const color = Number(hexColor);
  ...
  // Take the two most significant bits of each component
  const rr = (r >> 6) & 0b11;
  const gg = (g >> 6) & 0b11;
  const bb = (b >> 6) & 0b11;

  // Pack into 0b0rrggbb
  const packed = (rr << 4) | (gg << 2) | bb;

  // Add 1 before sending to the controller
  return packed + 1;
}
...
MC7000.padModeCue = function(channel, control, value, status, group) {
    ...
    for (let cueIdx = 1; cueIdx <= 8; cueIdx++) {
		...
		const hotcueColor = engine.getValue(group, `hotcue_${cueIdx}_color`);
        midi.sendShortMsg(0x94 + deckIndex, 0x14 + cueIdx - 1, hotcueEnabled ? MC7000.encodeColor(hotcueColor): MC7000.padColor.hotcueoff);
    }
};
...
MC7000.padModePitch = function(channel, control, value, status, group) {
    ...
    for (let pitchIdx = 1; pitchIdx <= 8; pitchIdx++) {
        if (MC7000.HotcueSelectedGroup[deckIndex] !== 0) {
			...
			const hotcueColor = engine.getValue(group, `hotcue_${pitchIdx}_color`);
            midi.sendShortMsg(0x94 + deckIndex, 0x14 + pitchIdx - 1, hotcueEnabled ? MC7000.encodeColor(hotcueColor) : MC7000.padColor.hotcueoff);
        }
    }
};
            ...
            for (let pitchIdx = 1; pitchIdx <= 8; pitchIdx++) {
                ...
                if (isButtonPressed && isControlAddress) {
                    if (!HotcueSelectedOnDeck) {
                        ...
                        midi.sendShortMsg(0x94 + deckIndex, 0x14 + pitchIdx - 1, MC7000.encodeColor(hotcueColor)); // if pitch is pressed switch to pitch on color
                        midi.sendShortMsg(0x94 + deckIndex, 0x1C + pitchIdx - 1, MC7000.padColor.encodeColor(hotcueColor)); // keep color when shift is pressed
                    }
                    ...
                    for (let padIdx = 1; padIdx <= 8; padIdx++) {
						...
						const hotcueColor = engine.getValue(group, `hotcue_${padIdx}_color`);
                        midi.sendShortMsg(0x94 + deckIndex, 0x14 + padIdx - 1, padHotcueEnabled ? MC7000.encodeColor(hotcueColor) : MC7000.padColor.hotcueoff);
                        midi.sendShortMsg(0x94 + deckIndex, 0x1C + padIdx - 1, padHotcueEnabled ? MC7000.encodeColor(hotcueColor) : MC7000.padColor.hotcueoff); // keep color when shift is pressed
                    }
                }
            }
        }
        ...
        for (let padIdx = 1; padIdx <= 8; padIdx++) {
            if (value === 1) {
                if (engine.getValue(group, `hotcue_${padIdx}_status`) === 1) {
					const hotcueColor = engine.getValue(group,`hotcue_${padIdx}_color`);
                    midi.sendShortMsg(0x94 + deckIndex, 0x14 + padIdx - 1, MC7000.encodeColor(hotcueColor));
                    midi.sendShortMsg(0x94 + deckIndex, 0x1C + padIdx - 1, MC7000.encodeColor(hotcueColor));
                }
            } else {
        ...
        for (let pitchIdx = 0; pitchIdx < 8; pitchIdx++) {
            if (engine.getValue("[Channel"+deckNumber+"]", "play") === 0) { // stopped
                if (MC7000.HotcueSelectedGroup[deckIndex] !== 0) { // hotcue selected
					...
					const hotcueColor = engine.getValue(group, `hotcue_${pitchIdx + 1}_color`);
                    midi.sendShortMsg(0x94 + deckIndex, 0x14 + pitchIdx, hotcueEnabled ? MC7000.encodeColor(hotcueColor) : MC7000.padColor.hotcueoff);
                }
            } else {
